  ...
  const menuBox = (
    <div ref={btnRef}>
      ...
      <div className={`speed-menu speed-dropdown-${placement}`} ref={menuRef}>
        {
          visible && menu.map(subMenu => {
            return <div className={menuClasses} onClick={() => { handleClickChange(subMenu) }} key={subMenu.key}>{subMenu.label}</div>
          })
        }
      </div>
    </div>
  )
  const childBox = React.cloneElement(child, {
