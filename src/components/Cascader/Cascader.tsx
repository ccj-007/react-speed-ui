...
const Cascader: FC<CascaderProps> = props => {
  ...
  const listener = (e: any) => {
    if (
      ...
      e.target.className !== 'speed-cascader-item' &&
      e.target.className !== 'speed-icon'
    ) {
      setVisible(false);
    }
  };
  ...
  const renderChild = (level: number) => {
    if (level === 0) {
      return optionsWarp.map((option: optionsType[0], id: number) => {
        return (
          <div className={`${prefixCls}-item`} onClick={() => showChildMenu(level, id)} style={getActiveStyles(option)}>
            {option.value}
      ...
      if (findChildObj.length > 0) {
        return findChildObj.map((obj: optionsType, id: number) => {
          return (
            <div className={`${prefixCls}-item`} onClick={() => handleEndNode(obj, id)} style={getActiveStyles(obj)}>
              {obj.value}
              {obj.children && obj.children.length > 0 && (
                <Icon icon={solid('angle-right')} size='1x' color={obj.isClicked ? iconActiveColor : '$4c4c4c'}></Icon>
              )}
            </div>
          );
        });
      ...
      <Transition in={visible} animation='zoom-in-top' addEndListener={() => {}} timeout={1000}>
        <div className={`${prefixCls}-content`}>
          {levels.map((level: number, index: number) => {
            return <div className={`${prefixCls}-column`}>{renderChild(level)}</div>;
          })}
        </div>
      </Transition>
    </div>
